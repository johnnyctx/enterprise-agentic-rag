...
generator: LLMProvider = build_llm(
...
def query(request: QueryRequest):
    ...
    """
    result = orchestrator.run(request.query, request.access_level)
    return {"answer": result["answer"]}


@app.post("/query/debug")
def query_debug(request: QueryRequest):
    """Developer endpoint with full RAG, validation, and diagnostic details."""
